...
for i = 1:S
    ...
    while n <= maxNumPackets
       ...
       %% compute the number of bits embeded in a packet
        ...
        rece_Sig{2} = [tmp_zeros;rece_Sig{2}];
        
       %%%%%*** Signal superposition from multiple tags ***%%%%%
        rxSig = complex(zeros(length(rece_Sig{2}),1));
        for rx_idx1 = 1:numTags
            [ttmp_l,ttmp_r] = size(rece_Sig{rx_idx1}');
            rxSig(ttmp_l:ttmp_r) = rxSig(ttmp_l:ttmp_r) + rece_Sig{rx_idx1};
        end
        %% add noise
