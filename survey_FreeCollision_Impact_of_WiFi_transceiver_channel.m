...
for i = 1:S
    ...
    while n <= maxNumPackets
       ...
       %% The modulated signal then propagates through the tag-to-receiver channel to the receiver. (considering multipath fading channel).
        for chan_tag_rx_idx1 = 1:numTags
            ...
            rece_Sig{chan_tag_rx_idx1} = tgnChannel_Tag_RX(bxSig{chan_tag_rx_idx1});
        end

        %%%%%*** transmitter-to-receiver channel ***%%%%%
       %% The excitation signal propagates through the transmitter-to-receiver channel to the WiFi receiver. (considering multipath fading channel).
        reset(tgnChannel_TX_RX)
        rxSig_from_WiFi_TX = tgnChannel_TX_RX(tx);
        
        WiFi_transceiver_link_snr = survey_FreeCollision_funcChannelQuality2SNR(WiFi_transceiver_link);
        ...
        %%%%%*** receiver side ***%%%%%
        [decodedPSDU,flag] = survey_FreeCollision_funcWiFiRX(rx_From_WiFi_TX,cfgHT);
        if flag == 0
            packetLoss(i,1) = packetLoss(i,1) + 1;
            for rx_idx2 = 1:numTags
                numBitErrs(i,rx_idx2) = numBitErrs(i,rx_idx2) + (numTagData-pLen);
            end
            n = n+1;
            continue;
        end
        ...
        % WiFi Receiver
        [~,ofdmSymDerived] = survey_FreeCollision_funcOFDMSymDerived(decodedPSDU,cfgHT);
        ...
        final_data = [];
        for rx_idx3 = 1:length(chanEst_data)
            tmp_L1 = chanEst_data(rx_idx3);
            tmp_Ym1 = ofdmDemodData(rx_idx3,:);
            tmp_Xm1 = ofdmDataDerived(rx_idx3,:);
        ...
        Q = [];
        for rx_idx4 = 2:(size(tagData,1)+1)
            tmp_A = final_data(:,rx_idx4);
            tmp_B = ofdmDataDerived(:,rx_idx4);
    ...
    for comm_idx3 = 1:numTags
        berEst(i,comm_idx3) = numBitErrs(i,comm_idx3)/((numTagData-pLen)*maxNumPackets);
    end
