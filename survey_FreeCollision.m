...
for i = 1:S
    ...
    while n <= maxNumPackets
        ...
        for chan_TX_Tags_idx1 = 1:numTags
        ...
        for tag_idx2 = 1:numTags
            bxSig{tag_idx2} = survey_FreeCollision_funcBackscatter(exSig(:,tag_idx2),tagData(:,tag_idx2),1);
        end
        
        H_Tags_RX = [];
        %%%%%** Tags-RX backscatter channel & AWGN **%%%%%
        for chan_Tags_RX_idx1 = 1:numTags
            ...
            rece_Sig{chan_Tags_RX_idx1} = bxSig{chan_Tags_RX_idx1}.*bx_coeff_TagRx;
        ...
        final_data = [];
        for rx_ii_1 = 1:length(chanEst_data)
            tmp_L1 = chanEst_data(rx_ii_1);
            tmp_Ym1 = ofdmDemodData(rx_ii_1,:);
            tmp_Xm1 = ofdmDataDerived(rx_ii_1,:);
        ...
        Q = [];
        for rx_ii_2 = 2:(size(tagData,1)+1)
            tmp_A = final_data(:,rx_ii_2);
            tmp_B = ofdmDataDerived(:,rx_ii_2);
        ...
        tmp_id = 1:numTags;
        for tt1 = 1:numTags
            target_data = demodData(:,tt1);
            tmp_errs = Inf(1,numTags);
            for tt2 = 1:length(tmp_id)
                tmp_errs(tmp_id(tt2)) = biterr(tagData(:,tmp_id(tt2)),target_data);
            end
            [~,est_tagID] = min(tmp_errs);
            tmp_id(tmp_id==est_tagID) = [];
            numBitErrs(i,tt1) = numBitErrs(i,tt1) + biterr(tagData(:,est_tagID),demodData(:,tt1));
        end
        n = n + 1;
    end
    for tt3 = 1:numTags
        berEst(i,tt3) = numBitErrs(i,tt3)/(numTagData*maxNumPackets);
    end
