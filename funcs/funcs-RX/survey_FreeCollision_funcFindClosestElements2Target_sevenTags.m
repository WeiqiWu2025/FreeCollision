function [estBaseH] = survey_FreeCollision_funcFindClosestElements2Target_sevenTags(channelSet,target)

tmp_min = abs(target-channelSet(1:7));
len = length(channelSet);
estBaseH = ones(7,1);
estBaseH = channelSet(1:7);
        ...
        for idx3 = (idx2+1):len
            for idx4 = (idx3+1):len
                for idx5 = (idx4+1):len
                    for idx6 = (idx5+1):len
                        for idx7 = (idx6+1):len
                            idxSet = [idx1,idx2,idx3,idx4,idx5,idx6,idx7];
